...
# Build a JSON array from args (safe for simple tokens like zone names)
json_array() {
  local first=1
  printf '['
  for it in "$@"; do
    ...
    # zone names won't contain quotes; simple print is fine
    printf '"%s"' "$it"
    first=0
  done
  printf ']'
...
detect_zones() {
  local zones_str=""
  if [ -n "${OFFPEAK_ZONES}" ]; then
    zones_str="${OFFPEAK_ZONES//,/ }"
    printf "[zones] Using OFFPEAK_ZONES from env: %s\n" "${zones_str}" >&2
  else
...
# Build array from the space-separated string
ZONES_ARR=()
for z in ${ZONES_STR}; do ZONES_ARR+=("$z"); done

ZONES_JSON="$(json_array "${ZONES_ARR[@]}")"

echo "[config] Applying off-peak policies to ${NP_SPOT} (cost) and ${NP_OD} (SLO)"

# Disruption patches (merge)
kubectl patch nodepool "${NP_SPOT}" --type=merge -p '{"spec":{"disruption":{"consolidationPolicy":"WhenEmptyOrUnderutilized"}}}'
kubectl patch nodepool "${NP_OD}"   --type=merge -p '{"spec":{"disruption":{"consolidationPolicy":"WhenEmpty","consolidateAfter":"60s"}}}'


# Helper to write a JSON patch file for a given template path (either /spec/template/spec or /spec/template)
write_req_patch() {
  local path_prefix="$1" # e.g., /spec/template/spec  OR  /spec/template
  local np="$2"         # nodepool name
  local outfile="$3"
  {
    printf '[{"op":"replace","path":"%s/requirements","value":[' "${path_prefix}"
    # First requirement: zone In [ZONES_JSON]
    printf '{"key":"topology.kubernetes.io/zone","operator":"In","values":%s}' "${ZONES_JSON}"
    printf ','
    # Second requirement: capacity-type
    if [ "$np" = "${NP_SPOT}" ]; then
...
# Apply requirements patch with primary path, then verify; if empty, retry with fallback path.
apply_and_verify() {
  local np="$1"
  ...
  echo "-------- PATCH CONTENT ($np / primary) --------"
  sed -n '1,200p' "${tmp}"
  echo "-----------------------------------------------"

  if ! kubectl patch nodepool "${np}" --type='json' --patch-file "${tmp}"; then
    echo "[warn] Primary path patch failed for $np, will try fallback (${fallback})"
  fi

  # Verify: read back the values from the primary path
  local got
  ...
  echo "[info] No requirements found at .spec.template.spec for $np, trying fallback path"
  write_req_patch "${fallback}" "${np}" "${tmp}"
  echo "-------- PATCH CONTENT ($np / fallback) -------"
  sed -n '1,200p' "${tmp}"
  echo "-----------------------------------------------"
  ...
  echo "[err] Requirements still not found on ${np}. Dumping spec snippet for debugging:"
...
echo "[ok] Off-peak profile applied (JSON Patch with path fallback)."
